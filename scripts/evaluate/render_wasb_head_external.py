"""Render fixed outcome transitions for the external head comparison."""
import hashlib
import json
...
def main():
    ...
    for row in review['selected_review']:
        ...
        with VideoFrameSequence(str(video)) as frames:
            ...
            cv2.putText(panel,f"EXTERNAL DEVELOPMENT {row['clip']} frame {index}: {row['kind']}",(7,16),cv2.FONT_HERSHEY_SIMPLEX,.36,(245,245,245),1)
            cv2.putText(panel,'Green label / red original / cyan head-adapted; training-selected thresholds',(7,35),cv2.FONT_HERSHEY_SIMPLEX,.4,(245,245,245),1)
    ...
    result=dict(complete=True,qualification_evidence=False,review_sha256=digest(out/'review.json'),script_sha256=digest(Path(__file__)),
                selection='First two per changed outcome transition, clip/frame order',entries=entries,image_hashes=hashes,visually_inspected=False)
