...
def main():
    ...
    folders = [base/'uvy_racket_supported_players_pilot01', base/'uvy_racket_supported_players_pilot02_nearest']
    ...
    with VideoFrameSequence(str(dataset/video)) as frames:
        ...
        try:
            for index, source in enumerate(frames):
                ...
                for column, (title, rows) in enumerate(zip(('ORIGINAL CROP OWNERSHIP', 'NEAREST PERSON CONSTRAINT'), tracks, strict=True)):
                    frame = source.copy()
    ...
    metadata = {'complete': True, 'qualification_evidence': False, 'frames': count, 'decoded_frames': decoded,
                ...
                'scope': 'FULL_SEQUENCE_PAIRED_RESEARCH_REVIEW; NO_COURT_ROLES_OR_PHYSICAL_ANALYTICS'}
