"""Replay photometric filter ablations against every saved court case."""
import argparse
import json
...
from scripts.evaluate.refine_court_line_geometry import refine
from scripts.evaluate.probe_court_ridge_ablation import propose as ablation_propose
...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--output',type=Path,required=True);parser.add_argument('--mode',choices=['no_white','no_sides','ridge_only'],required=True);args=parser.parse_args()
    ...
    report=dict(mode=args.mode,complete=False,qualification_evidence=False,scope='REUSED_DEVELOPMENT_COMPARISON; NOT_INDEPENDENT_ACCURACY',sources=sources,
        protocol_sha256=digest(ROOT/'docs/experiments/COURT_RIDGE_ABLATION_PROTOCOL.md'),
        code_hashes={p:digest(ROOT/p) for p in ['scripts/evaluate/benchmark_court_ridge_ablation.py','scripts/evaluate/probe_court_ridge_ablation.py','scripts/evaluate/refine_court_line_geometry.py','scripts/evaluate/probe_court_line_segments.py','scripts/evaluate/audit_court_line_support.py','scripts/evaluate/audit_caltennis_projection.py','scripts/evaluate/benchmark_court_refinement_labels.py']},cases=[])
    cache={}
    for index,(group,old) in enumerate(cases):
        ...
        if not valid:
            ...
            if group!='pilot':entry['scores']={name:old['scores']['projected'] for name in ('projected','original','ablation')}
        ...
        ablation_segments=[x['segment'] for x in ablation_propose(image,args.mode) if x['support_fraction']>=.5]
        actual,evidence=refine(points,ablation_segments,image.shape[1],image.shape[0])
        entry.update(initial_valid=True,accepted=evidence['accepted'],reason=evidence['reason'],evidence=evidence,segments=ablation_segments,baseline_segment_count=len(segments),
            original_points=points.tolist(),previous_refined_points=expected.tolist(),ablation_refined_points=actual.tolist(),
        ...
        if labels is not None:
            entry['scores']={name:score((p/.75).tolist(),labels) for name,p in [('projected',points),('original',expected),('ablation',actual)]}
        ...
        panels=[]
        for name,candidate in [('Projected',points),('Previous refinement',expected),(args.mode,actual)]:
    ...
    for group in ('selection','external_source_check'):
        pooled[group]={}
        for stage in ('projected','original','ablation'):
