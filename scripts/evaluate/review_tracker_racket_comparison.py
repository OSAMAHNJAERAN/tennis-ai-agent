"""Verify paired tracker artifacts, summarize identity coverage and render review."""
import json
...
def main():
    ...
    old_path = base/'uvy_trained_racket_players_pilot01/report.json'
    ...
    result = {'complete': False, 'qualification_evidence': False, 'report_sha256': digest(report_path),
        'person_report_sha256': digest(people_path), 'previous_trained_report_sha256': digest(old_path),
            ...
            for variant, candidate in value['variants'].items():
                ...
                if variant == 'flow_handoff':
                    ...
                    if kind == 'bytetrack':
                        ...
                        entry['previous_trained_byte_exact_frames'] = sum(a == b for a, b in zip(selected, previous, strict=True))
    ...
    print(json.dumps({'aggregate': result['aggregate'], 'videos': result['videos'], 'previous_byte_exact': {s: e['previous_trained_byte_exact_frames'] for s,e in result['sequences'].items()}}, indent=2))
