...
def main():
    ...
    selected=[r for r in manifest['samples'] if r['partition']=='external_source_check'];assert len(selected)==8
    ...
    report={'complete':False,'qualification_evidence':False,'scope':'RAW_PUBLISHER_LABEL_AGREEMENT; LABEL_ERRORS_KNOWN; RESNET_TRAINING_INDEPENDENCE_UNKNOWN',
            ...
            'protocol_sha256':digest(ROOT/'docs/experiments/COURT_REFINEMENT_RAW_LABEL_PROTOCOL.md'),
            'code_hashes':{p:digest(ROOT/p) for p in ('scripts/evaluate/benchmark_court_refinement_labels.py',
