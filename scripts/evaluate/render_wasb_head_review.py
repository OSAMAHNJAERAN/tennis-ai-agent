"""Render the fixed paired-outcome sample for the head adaptation pilot."""
import hashlib
import json
...
def main():
    ...
    result=dict(complete=True,qualification_evidence=False,review_sha256=digest(out/'review.json'),script_sha256=digest(Path(__file__)),
                selection='First two per gained/lost visible/absence category, clip/frame order',entries=entries,image_hashes=hashes,visually_inspected=False)
