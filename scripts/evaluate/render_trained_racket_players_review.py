...
def main():
    ...
    folders = [base/'uvy_duplicate_handoff_pilot01', base/'uvy_trained_racket_players_pilot01']
    ...
    with VideoFrameSequence(str(dataset/video)) as frames:
        ...
        try:
            for index, source in enumerate(frames):
                ...
                for column, (title, rows) in enumerate(zip(('ORIGINAL RACKET MODEL', 'TRAINED RACKET MODEL'), tracks, strict=True)):
                    frame = source.copy()
    ...
    metadata = {'complete': True, 'qualification_evidence': False, 'frames': count, 'decoded_frames': decoded,
                ...
                'scope': 'FULL_SEQUENCE_TRAINED_RACKET_RESEARCH_REVIEW; NO_COURT_ROLES_OR_PHYSICAL_ANALYTICS'}
