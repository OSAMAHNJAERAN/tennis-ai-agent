"""Resume an interrupted verified RT-DETR benchmark at complete sequence boundaries."""
import argparse
import json
import os
...
def main():
    ...
    if previous['complete']:
        raise ValueError('Benchmark already complete')
    ...
    for key in ('checkpoint_sha256', 'checkpoint_manifest_sha256', 'dataset_manifest_sha256',
                'baseline_report_sha256', 'protocol_sha256', 'configuration', 'tracker_configs',
                'runtime', 'installed_source_hashes'):
        if previous[key] != report[key]:
            raise ValueError(f'Resume state differs: {key}')
    if digest(args.output/'frozen_protocol.md') != previous['protocol_sha256']:
        raise ValueError('Frozen protocol changed')
    for entry in previous['sequences'].values():
        if digest(args.output/entry['raw_file']) != entry['raw_sha256']:
            raise ValueError('Completed raw cache changed')
        for tracked in entry['trackers'].values():
            if digest(args.output/tracked['predictions_file']) != tracked['predictions_sha256']:
                raise ValueError('Completed tracker cache changed')
    snapshot = args.output/'report_before_resume.json'
    if not snapshot.exists():
        snapshot.write_bytes((args.output/'report.json').read_bytes())
    report = previous
    report.setdefault('resume_audit', []).append({'prior_report_sha256': digest(args.output/'report.json'),
        'resume_script_sha256': digest(Path(__file__)), 'retained_sequences': list(report['sequences']),
        'reason': 'Prior process handle missing and OS process absent; only complete sequence caches reused'})
    def save():
    ...
    for sequence, info in manifest['sequences'].items():
        if sequence in report['sequences']:
            print(json.dumps({'stage': 'reuse_complete_raw', 'sequence': sequence}), flush=True)
            continue
        if (args.output/f'{sequence}_raw.json').exists():
            raise ValueError('Unrecorded raw cache requires inspection')
        raw, degenerate = [], 0
