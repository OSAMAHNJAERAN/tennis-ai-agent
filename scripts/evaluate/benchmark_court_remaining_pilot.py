"""Compare fixed court refinement with unchanged, known-imperfect raw labels."""
import argparse
import json
...
from scripts.evaluate.refine_court_line_geometry import refine as old_refine, LINES
from scripts.evaluate.refine_court_boundary_geometry import refine
from scripts.evaluate.probe_court_ridge_ablation import propose as ridge_propose
...
def main():
    ...
    selected=[r for r in manifest['samples'] if r['partition']=='train'];assert len(selected)==125
    refinement_source=ROOT/'outputs/vision_upgrade_audit/court_ridge_boundary01/report.json'
    ...
    report={'complete':False,'qualification_evidence':False,'scope':'REMAINING_PILOT_TRAIN_IMAGES; PREVIOUS_HEATMAP_TRAINING_EXPOSURE; NOT_INDEPENDENT_QUALIFICATION',
            ...
            'protocol_sha256':digest(ROOT/'docs/experiments/COURT_REMAINING_PILOT_PROTOCOL.md'),
            'code_hashes':{p:digest(ROOT/p) for p in ('scripts/evaluate/benchmark_court_remaining_pilot.py','scripts/evaluate/benchmark_court_refinement_labels.py','scripts/evaluate/refine_court_boundary_geometry.py','scripts/evaluate/probe_court_ridge_ablation.py',
    ...
    for row in selected:
        ...
        projected=final=original=[None]*14
        ...
        if calibration.is_valid:
            ...
            old_points,original_evidence=old_refine(projected*.75,segments)
            original=(old_points/.75).tolist()
            combined_segments=[r['segment'] for r in ridge_propose(reference,'no_white') if r['support_fraction']>=.5]
            refined,evidence=refine(projected*.75,combined_segments)
            final=(refined/.75).tolist();projected=projected.tolist()
        # Labels enter only below this point, after all prediction decisions.
        variants={'raw':raw.tolist(),'projected':projected,'original':original,'refined':final}
        ...
        entry={'id':row['id'],'input_sha256':digest(path),'initial_calibration':calibration.to_dict(),
               'refinement':evidence,'original_refinement':original_evidence,'scores':scored,'image':output.name,'image_sha256':digest(output),'visually_inspected':False}
    ...
    pooled={}
    for name in ('raw','projected','original','refined'):
