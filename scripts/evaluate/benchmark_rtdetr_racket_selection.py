"""RT-DETR person proposals with the frozen original-racket player selector."""
import argparse
import json
import math
import os
...
def main():
    ...
    parser.add_argument('--person-report', type=Path, default=ROOT/'outputs/vision_upgrade_audit/rtdetr_player_proposals_pilot01/report.json')
    ...
    if digest(ROOT/'yolo11m.pt') != training['initial_checkpoint_sha256']:
        raise ValueError('Original checkpoint changed')
    if person['checkpoint_sha256'] != '6de60b10d4bc566f00cda0f5b4d64afe4b66d48dc9695d2171effb7859d8e73f':
        raise ValueError('RT-DETR checkpoint differs')
    for event in person.get('resume_audit', []):
        if digest(ROOT/'scripts/evaluate/resume_rtdetr_player_proposals.py') != event['resume_script_sha256']:
    ...
    cv2.setNumThreads(1)
    protocol = ROOT/'docs/experiments/RTDETR_RACKET_PLAYER_SELECTION.md'
    sources = ['scripts/evaluate/benchmark_rtdetr_racket_selection.py', 'scripts/evaluate/benchmark_racket_pilot04_restricted.py',
    ...
    report = {'complete': False, 'qualification_evidence': False, 'person_report_sha256': digest(person_path),
        ...
        'configuration': {'racket_model': 'ORIGINAL_YOLO11M', 'sample_seconds': .2, 'racket_confidence': .25, 'racket_imgsz': 640,
