...
def main():
    ...
    folders = [base/'uvy_flow_fragment_linking_pilot01', base/'uvy_duplicate_handoff_pilot01']
    ...
    with VideoFrameSequence(str(dataset/video)) as frames:
        ...
        try:
            for index, source in enumerate(frames):
                ...
                for column, (title, rows) in enumerate(zip(('BEFORE DUPLICATE HANDOFF', 'AFTER DUPLICATE HANDOFF'), tracks, strict=True)):
                    frame = source.copy()
    ...
    metadata = {'complete': True, 'qualification_evidence': False, 'frames': count, 'decoded_frames': decoded,
                ...
                'scope': 'FULL_SEQUENCE_DUPLICATE_HANDOFF_RESEARCH_REVIEW; NO_COURT_ROLES_OR_PHYSICAL_ANALYTICS'}
